...
impl PortAgent {
    ...
    fn add_port(&self, name: &str) -> zbus::fdo::Result<String> {
        ...
        let naming_template = "vi_{container}".to_string();
        let vmid: u32 = 0;
    ...
    fn get_blocks_by_category(&self, category: &str) -> zbus::fdo::Result<String> {
        ...
        let blocks = ledger.get_blocks_by_category(category)
    ...
    fn get_blocks_by_height(&self, start: u64, end: u64) -> zbus::fdo::Result<String> {
        ...
        let blocks = ledger.get_blocks_by_height(start, end)
            .map_err(|e| zbus::fdo::Error::Failed(format!("Failed to get blocks: {}", e)))?;
    ...
    fn verify_blockchain_integrity(&self) -> zbus::fdo::Result<String> {
        ...
        Ok(if is_valid { "Blockchain integrity: VALID".to_string() } else { "Blockchain integrity: INVALID".to_string() })
    }

    /// Add data to blockchain through D-Bus
    fn add_blockchain_data(&self, category: &str, action: &str, data: &str) -> zbus::fdo::Result<String> {
        ...
        let block_hash = ledger.add_data(category, action, data_value)
            .map_err(|e| zbus::fdo::Error::Failed(format!("Failed to add data: {}", e)))?;

        Ok(format!("Data added to blockchain with hash: {}", block_hash))
    }
    ...
    fn get_block_by_hash(&self, hash: &str) -> zbus::fdo::Result<String> {
        ...
        let block = ledger.get_block(hash)
    ...
    if let Ok(output) = tokio::process::Command::new("ovs-vsctl")
        ...
        for bridge_name in bridge_names.lines() {
            ...
            if let Ok(output) = tokio::process::Command::new("ovs-vsctl")
                ...
                bridge_state.ports = ports.lines()
            ...
            if let Ok(output) = tokio::process::Command::new("ovs-vsctl")
                ...
                bridge_state.interfaces = interfaces.lines()
...
async fn validate_bridge_connectivity(bridge: &str) -> Result<serde_json::Value> {
    ...
    validation.insert("connectivity_preserved".to_string(), json!(connectivity_preserved));
...
async fn perform_atomic_bridge_operation(bridge: &str, operation: &str) -> Result<String> {
    debug!("Performing atomic bridge operation: {} on {}", operation, bridge);
    ...
    match operation {
        ...
        "validate_topology" => {
            ...
            Ok(format!("Topology validation: {}", if validation { "PASSED" } else { "FAILED" }))
        }
        "sync_with_proxmox" => {
        ...
        _ => {
            Err(anyhow::anyhow!("Unknown atomic operation: {}", operation))
        }
    }
}
...
async fn get_bridge_topology(bridge: &str) -> Result<serde_json::Value> {
    ...
    if let Ok(output) = tokio::process::Command::new("ovs-vsctl")
        ...
        .output().await {
        let ports = String::from_utf8_lossy(&output.stdout);
        topology.insert("ports".to_string(), json!(ports.lines().filter(|p| !p.trim().is_empty()).collect::<Vec<_>>()));
    }
    ...
    if let Ok(output) = tokio::process::Command::new("ovs-vsctl")
        ...
        .output().await {
        let interfaces = String::from_utf8_lossy(&output.stdout);
        topology.insert("interfaces".to_string(), json!(interfaces.lines().filter(|i| !i.trim().is_empty()).collect::<Vec<_>>()));
    }
